...
class RaceSimulation(tf_agents.environments.py_environment.PyEnvironment):
    ...
    def _step(self, action) -> tf_agents.trajectories.time_step.TimeStep:
        ...
        # evaluate actions ---------------------------------------------------------------------------------------------
        position_before_simulate_lap = self.race.positions[self.race.cur_lap, self.idx_driver]
        laptime_before_pitstop = self.race.laptimes[self.race.cur_lap, self.idx_driver]

        if action in [1, 2, 3]:
            # pit stop in this lap, compound = available_compounds[action - 1]
            self.race.drivers_list[self.idx_driver].strategy_info.\
                append([self.race.cur_lap, self.available_compounds[action - 1], 0, 0.0])

        # simulate next lap
        self.race._RaceReinftrain__simulate_lap()

        # positions lost or won during pit stop entrance and end of the lap
        position_after_simulate_lap = self.race.positions[self.race.cur_lap - 1, self.idx_driver]
        ...
        laptime_after_pitstop = self.race.laptimes[self.race.cur_lap - 1, self.idx_driver]
        ...
        reward = 0.1 * (self.__calculate_reward_laptime(t_pitdrive_inlap)
                        + self.__calculate_reward_position(delta_position))
        ...
        if self._episode_ended:
            ...
            # reward for the final position
            reward += self.__calculate_reward_final_position()

            # at least two different compounds used during the race
            used_2compounds = (True if len({x[1] for x in self.race.drivers_list[self.idx_driver].strategy_info}) > 1
                               else False)
